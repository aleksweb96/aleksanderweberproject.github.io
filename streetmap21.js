	...
	function getMaleColor() {
        var maleColor = $('#maleColor').val() == undefined ? MALE_COLOR : $('#maleColor').val();
        return maleColor;
    }

    function getFemaleColor() {
        var maleColor = $('#femaleColor').val() == undefined ? FEMALE_COLOR : $('#femaleColor').val();
        return maleColor;
	}
	...
	function drawLineChart(dataArray, 
		...
		function mapToggle(ptNode, d, i, deathDatesToShow) {
            ...
            for (var j = 0; j < deathsObjects.length; j++) {
                ...
                deathCircle.attr("opacity", newVal)
                    .attr("class",
                        function(d) {
                            var id = "id_" + j;
                            var sex = (deathsObjects[j].sex == 0 ? "male" : "female");
                            var age = "age" + deathsObjects[j].age;
                            var dateStr = "date_" + deathsObjects[j].date;
	...
	function createDeathCircles(mapGroup, deathData) {
		...
		var deathCircles = mapGroup.selectAll("circle .death") 
			...
			.attr("class", function(d, i) {
                var id = "id_" + i;
				var sex = (d.sex == 0 ?  "male"  : "female");
				var age = "age" + d.age;
				var dateStr = "date_" + d.date;
                var classString = "death" + " " + id + " " + sex + " " + age + " " + dateStr; 
				return classString;
			})
			;
	...
	$(document).ready(function () {
		...
		setChangeHandler('#age0Opacity', '.age0', 'opacity', '1.0');
		setChangeHandler('#age1Opacity', '.age1', 'opacity', '1.0');
		setChangeHandler('#age2Opacity', '.age2', 'opacity', '1.0');
		setChangeHandler('#age3Opacity', '.age3', 'opacity', '1.0');
		setChangeHandler('#age4Opacity', '.age4', 'opacity', '1.0');
		setChangeHandler('#age5Opacity', '.age5', 'opacity', '1.0');
